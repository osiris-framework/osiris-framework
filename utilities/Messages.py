...
class Messages(object):
    ...
    def start_execution(self):
        ...
        print(color.color("yellow", "[!] ") + color.color("lgray", "Starting ") + color.color("blue", self.__category) + " " +  color.color("green", self.__module) + color.color("red", " Time: ") + color.color("green", self.__local_time))

    def end_execution(self):
        self.__category = self.name_module.split('/')[-4]
        self.__module = self.name_module.split('/')[-1].split('.')[0]
        self.__local_time = time.asctime(time.localtime(time.time()))
        print(color.color("blue","[*] ") + color.color("green", self.__category) + " " + color.color("lgray","Module execution completed ") + color.color("red", "Time: ") + color.color("green",self.__local_time))

    def execution_info(self, message):
        self.__local_time = time.asctime(time.localtime(time.time()))
        print(color.color("yellow", "[!] INFO ") + color.color("green", "{}".format(message)) + color.color("red", " Time: ") + color.color("green",self.__local_time))

    def execution_warning(self, message):
        self.__local_time = time.asctime(time.localtime(time.time()))
        print(color.color("yellow", "[!] WARNING ") + color.color("lgray", "{}".format(message)) + color.color("red", " Time: ") + color.color("green",self.__local_time))

    def execution_error(self, message):
        self.__local_time = time.asctime(time.localtime(time.time()))
        print(color.color("red", "[-] ERROR ") + color.color("yellow", "{}".format(message)) + color.color("red", " Time: ") + color.color("green",self.__local_time))
